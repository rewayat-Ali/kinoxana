...
const App = {
    ...
    if (playBtn && !playBtn._bound) {
      ...
      playBtn.addEventListener('click', () => {
        ...
        if (this.state.currentMovie) {
          this.closeDetail();
          setTimeout(() => {
            this.openPlayer(this.state.currentMovie.id, this.state.currentEp || 1);
          }, 300);
        }
      });
    }
  ...
  selectEpisode(epNum) {
    ...
    this.closeDetail();
    setTimeout(() => {
      this.openPlayer(this.state.currentMovie.id, epNum);
    }, 300);
  },
  ...
  normalizeMovie(m) {
    ...
    return {
      ...
      totalEpisodes: m.totalEpisodes || m.episodes || m.total_episodes || m.episode_count,
  ...
  openPlayer(movieId, epNum = null) {
    const movie = this.state.currentMovie && this.state.currentMovie.id === movieId
    ...
    const open = (m) => {
      ...
      this._renderPlayerInfo(m);
      this._setupVideo();
      this._renderPlayerEpisodes(m);
      const startEp = epNum || this.state.currentEp || 1;
      this.player.currentEp = startEp;
      this._playEpisode(startEp);
      this.addHistory(m.id);
    };
    if (movie) {
      open(movie);
    } else {
      Api.getMovie(movieId).then(res => {
        const m = this.normalizeMovie(res);
        this.state.currentMovie = m;
        open(m);
      }).catch(err => {
        this.showToast(err.message || 'يۈكلىيەلمىدى', 'error');
      });
    }
  },
  ...
  closePlayer() {
    ...
    const screen = document.getElementById('player-screen');
    screen.style.animation = 'playerSlideUp 0.3s reverse ease forwards';
    setTimeout(() => {
  ...
  _renderPlayerInfo(m) {
    document.getElementById('player-title').textContent = m.title || 'نامسىز';
    document.getElementById('player-info-title').textContent = m.title || 'نامسىز';
    const yearEl = document.getElementById('player-info-year');
    const ratingEl = document.getElementById('player-info-rating');
    const durEl = document.getElementById('player-info-duration');
    yearEl.textContent = m.year ? m.year : '';
    yearEl.style.display = m.year ? '' : 'none';
    ratingEl.textContent = m.rating ? `★ ${Number(m.rating).toFixed(1)}` : '';
    ratingEl.style.display = m.rating ? '' : 'none';
    durEl.textContent = m.duration ? `${m.duration} مىنۇت` : '';
    durEl.style.display = m.duration ? '' : 'none';
    document.getElementById('player-info-desc').textContent = m.description || 'چۈشەندۈرۈش يوق';
    const tagsEl = document.getElementById('player-info-tags');
    if (m.categoryNames && m.categoryNames.length > 0) {
      tagsEl.innerHTML = m.categoryNames.map(n => `<span class="tag">${this.escapeHtml(n)}</span>`).join('');
      tagsEl.style.display = 'flex';
    } else {
      tagsEl.style.display = 'none';
    }
    const poster = m.backdrop || m.poster || '';
  ...
  _setupVideo() {
    ...
    video.addEventListener('play', () => {
      ...
      document.getElementById('player-center-play').classList.add('hidden');
    });
    video.addEventListener('pause', () => {
      this.player.isPlaying = false;
      this._updatePlayIcon(false);
      document.getElementById('player-center-play').classList.remove('hidden');
    });
    ...
    video.addEventListener('error', () => {
      document.getElementById('player-loading').style.display = 'none';
      this.showToast('ۋىدېيو قويۇش مەغلۇب بولدى', 'error');
    });
  },

  _renderPlayerEpisodes(m) {
    const panel = document.getElementById('player-episodes-panel');
    const catNames = (m.categoryNames || []).join(' ').toLowerCase();
    const title = (m.title || '').toLowerCase();
    const isSeries = /tv|series|تېلېۋىزىيە|电视剧|پەسلىل|连续剧|drama|cartoon|كارتۇن|动漫|动画|anime/.test(catNames + ' ' + title);
    const hasEpisodes = m.totalEpisodes && m.totalEpisodes > 1;
    ...
    if (!isSeries && !hasEpisodes) {
      ...
      this.player.episodes = [];
      return;
    }

    panel.style.display = 'block';
    const epCount = hasEpisodes ? m.totalEpisodes : 12;
    const vipStart = Math.floor(epCount * 0.4);
    const episodes = [];
    ...
    this.player.episodes = episodes;
    this.player.epFilter = 'all';

    document.getElementById('player-ep-count').textContent = `${epCount} قىسىم`;
    this._renderPlayerEpGrid(episodes);
    this._bindPlayerEpFilter();
  },

  _renderPlayerEpGrid(episodes) {
    const grid = document.getElementById('player-ep-grid');
    grid.innerHTML = episodes.map(ep => `
      <div class="episode-item ${ep.vip ? 'vip' : ''} ${ep.played ? 'played' : ''} ${this.player.currentEp === ep.num ? 'playing' : ''}"
           data-ep="${ep.num}">
        <span class="ep-num">${ep.num}</span>
      </div>
    `).join('');
    grid.querySelectorAll('.episode-item').forEach(item => {
      item.addEventListener('click', () => {
        const epNum = parseInt(item.dataset.ep);
        this._selectPlayerEpisode(epNum);
      });
    });
  },
  ...
  _bindPlayerEpFilter() {
    const panel = document.getElementById('player-episodes-panel');
    panel.querySelectorAll('.ep-filter-btn').forEach(btn => {
      if (btn._bound) return;
      btn._bound = true;
      btn.addEventListener('click', () => {
        this.player.epFilter = btn.dataset.filter;
        panel.querySelectorAll('.ep-filter-btn').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
        let filtered = this.player.episodes;
        if (btn.dataset.filter === 'free') filtered = this.player.episodes.filter(e => !e.vip);
        if (btn.dataset.filter === 'vip') filtered = this.player.episodes.filter(e => e.vip);
        this._renderPlayerEpGrid(filtered);
      });
    });
  },

  _selectPlayerEpisode(epNum) {
    const ep = this.player.episodes.find(e => e.num === epNum);
    if (!ep) return;
    if (ep.vip) {
      this.showToast('بۇ قىسىم VIP ئەزا ئىشلىتىشىگە خاس', 'error');
      return;
    }
    this.player.currentEp = epNum;
    document.getElementById('player-ep-label').textContent = `${epNum}-قىسىم`;
    this._playEpisode(epNum);
    this._renderPlayerEpGrid(this.player.epFilter === 'all'
      ? this.player.episodes
      : this.player.episodes.filter(e =>
          this.player.epFilter === 'free' ? !e.vip : e.vip));
  },

  _playEpisode(epNum) {
    const video = this.player.video;
    if (!video) return;
    const m = this.player.movie;
    document.getElementById('player-ep-label').textContent =
      this.player.episodes.length > 0 ? `${epNum}-قىسىم` : '';
    document.getElementById('player-loading').style.display = 'flex';
    const src = this._getVideoSrc(m, epNum);
    if (src) {
      video.src = src;
      video.load();
      video.play().catch(() => {});
    } else {
      this.showToast('ۋىدېيو مەنبەسى تېپىلمىدى', 'error');
      document.getElementById('player-loading').style.display = 'none';
    }
  },

  _getVideoSrc(movie, epNum) {
    if (movie.sources) {
      if (Array.isArray(movie.sources)) {
        const idx = Math.min(epNum - 1, movie.sources.length - 1);
        return typeof movie.sources[idx] === 'string' ? movie.sources[idx] : (movie.sources[idx].url || '');
      }
      return movie.sources;
    }
    if (movie.videoUrl) return movie.videoUrl;
    if (movie.source_url) return movie.source_url;
    return '';
  },

  _onVideoEnded() {
    const next = this.player.currentEp + 1;
    const maxEp = this.player.episodes.length;
    if (maxEp > 0 && next <= maxEp) {
      const nextEp = this.player.episodes.find(e => e.num === next);
      if (nextEp && !nextEp.vip) {
        this._selectPlayerEpisode(next);
      } else {
        this.showToast('كېيىنكى قىسىم VIP ئەزا ئىشلىتىشىگە خاس', 'error');
      }
    }
  },
  ...
  _updateProgress() {
    ...
    document.getElementById('player-played').style.width = pct + '%';
    document.getElementById('player-thumb').style.insetInlineStart = pct + '%';
    document.getElementById('player-progress-input').value = pct;
    document.getElementById('player-cur-time').textContent = this._formatTime(video.currentTime);
  },
  ...
  _updateBuffered() {
    ...
    document.getElementById('player-buffered').style.width = pct + '%';
  },

  _updateTotalTime() {
    const video = this.player.video;
    if (!video || !video.duration) return;
    document.getElementById('player-total-time').textContent = this._formatTime(video.duration);
  },
  ...
  toggleMute() {
    ...
    const volInput = document.getElementById('player-vol-input');
    if (video.muted) {
      volInput.value = 0;
    } else {
      volInput.value = Math.round(video.volume * 100);
    }
  },
  ...
  toggleSpeed() {
    ...
    document.getElementById('player-speed-label').textContent = next + 'x';
  },
  ...
  nextEpisode() {
    ...
    if (maxEp === 0) return;
    const next = this.player.currentEp + 1;
    if (next <= maxEp) this._selectPlayerEpisode(next);
  },

  prevEpisode() {
    if (this.player.episodes.length === 0) return;
    const prev = this.player.currentEp - 1;
    if (prev >= 1) this._selectPlayerEpisode(prev);
  },

  toggleFullscreen() {
    const screen = document.getElementById('player-screen');
    const isFs = screen.classList.contains('fullscreen');
    if (isFs) {
      screen.classList.remove('fullscreen');
  ...
  _bindPlayerEvents() {
    ...
    if (btnPlay && !btnPlay._bound) {
      btnPlay._bound = true;
      btnPlay.addEventListener('click', () => this.togglePlay());
    }
    const centerPlay = document.getElementById('player-center-play');
    if (centerPlay && !centerPlay._bound) {
      centerPlay._bound = true;
      centerPlay.addEventListener('click', () => this.togglePlay());
    }
    const videoEl = document.getElementById('player-video');
    if (videoEl && !videoEl._bound) {
      videoEl._bound = true;
      videoEl.addEventListener('click', () => this.togglePlay());
    }
    ...
    const progress = document.getElementById('player-progress-input');
    if (progress && !progress._bound) {
      progress._bound = true;
      progress.addEventListener('input', (e) => {
        const val = parseFloat(e.target.value);
        document.getElementById('player-played').style.width = val + '%';
        document.getElementById('player-thumb').style.insetInlineStart = val + '%';
      });
    ...
    if (volBtn && !volBtn._bound) {
      volBtn._bound = true;
      volBtn.addEventListener('click', () => this.toggleMute());
    }
    const volInput = document.getElementById('player-vol-input');
    if (volInput && !volInput._bound) {
      volInput._bound = true;
      volInput.addEventListener('input', (e) => this.setVolume(parseFloat(e.target.value)));
    }
    const speedBtn = document.getElementById('player-speed-btn');
    if (speedBtn && !speedBtn._bound) {
      speedBtn._bound = true;
    ...
    if (prevBtn && !prevBtn._bound) {
      prevBtn._bound = true;
      prevBtn.addEventListener('click', () => this.prevEpisode());
    }
    ...
    const castBtn = document.getElementById('player-cast');
    if (castBtn && !castBtn._bound) {
      castBtn._bound = true;
      castBtn.addEventListener('click', () => this.showToast('پروېكتورلاش مۇلازىمىتى تېخى ئېچىلمىدى', 'error'));
    }
    const epBtn = document.getElementById('player-ep-btn');
    if (epBtn && !epBtn._bound) {
      epBtn._bound = true;
      epBtn.addEventListener('click', () => {
        document.getElementById('player-episodes-panel').scrollIntoView({ behavior: 'smooth' });
      });
    }
  },
};
